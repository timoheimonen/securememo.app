...
if (!message) {
showMessage('메모를 입력해 주세요', 'error');
return;
}
if (message.length > 10000) {
showMessage('메모가 너무 깁니다(최대 10,000자)', 'error');
return;
}
...
if (err) {
showMessage('보안 인증을 완료해 주세요', 'error');
...
showMessage(result.error || '메모 생성에 실패했습니다', 'error');
}
}
} catch (error) {
showMessage('메모를 생성하는 동안 오류가 발생했습니다', 'error');
} finally {
submitButton.disabled = false;
submitButton.textContent = '보안 메모 작성';
...
showMessage('⚠️ 수동으로 복사해 주세요 (Ctrl+C / Cmd+C)', 'warning');
}
});
...
showMessage('✅ 비밀번호가 클립보드에 복사되었습니다!', 'success');
