...
if (!message) {
showMessage('Pakilagay ang isang memo', 'error');
return;
}
if (message.length > 10000) {
showMessage('Masyadong mahaba ang memo (max 10,000 karakter)', 'error');
return;
}
...
if (err) {
showMessage('Pakikumpleto ang security challenge', 'error');
...
submitButton.textContent = 'Ini-encrypt...';
...
showMessage(result.error || 'Nabigo ang paglikha ng memo', 'error');
}
}
} catch (error) {
showMessage('May naganap na error habang lumilikha ng memo', 'error');
} finally {
submitButton.disabled = false;
submitButton.textContent = 'Gumawa ng Ligtas na Memo';
...
showMessage('✅ Nakopya ang URL sa clipboard!', 'success');
...
showMessage('⚠️ Mangyaring kopyahin nang mano-mano (Ctrl+C / Cmd+C)', 'warning');
}
});
...
showMessage('✅ Nakopya ang password sa clipboard!', 'success');
