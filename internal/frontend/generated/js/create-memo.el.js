...
if (!message) {
showMessage('Παρακαλώ εισάγετε μια σημείωση', 'error');
return;
}
if (message.length > 10000) {
showMessage('Η σημείωση είναι πολύ μεγάλη (μέγιστο 10.000 χαρακτήρες)', 'error');
return;
}
...
if (err) {
showMessage('Παρακαλώ ολοκληρώστε την πρόκληση ασφαλείας', 'error');
...
submitButton.textContent = 'Κρυπτογράφηση...';
...
showMessage(result.error || 'Αποτυχία δημιουργίας σημείωσης', 'error');
}
}
} catch (error) {
showMessage('Προέκυψε σφάλμα κατά τη δημιουργία της σημείωσης', 'error');
} finally {
submitButton.disabled = false;
submitButton.textContent = 'Δημιουργία Ασφαλούς Σημείωσης';
...
showMessage('✅ Το URL αντιγράφηκε στο πρόχειρο!', 'success');
...
showMessage('⚠️ Παρακαλώ αντιγράψτε χειροκίνητα (Ctrl+C / Cmd+C)', 'warning');
}
});
...
showMessage('✅ Ο κωδικός αντιγράφηκε στο πρόχειρο!', 'success');
