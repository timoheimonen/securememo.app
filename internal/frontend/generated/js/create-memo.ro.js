...
if (!message) {
showMessage('Te rugăm să introduci un memo', 'error');
return;
}
...
submitButton.textContent = 'Se criptează...';
...
showMessage(result.error || 'Crearea memoului a eșuat', 'error');
}
}
} catch (error) {
showMessage('A apărut o eroare la crearea memoului', 'error');
} finally {
submitButton.disabled = false;
submitButton.textContent = 'Creează memo securizat';
...
showMessage('⚠️ Te rugăm să copiezi manual (Ctrl+C / Cmd+C)', 'warning');
}
});
...
showMessage('✅ Parola a fost copiată în clipboard!', 'success');
