...
showMessage('メモが長すぎます（最大10,000文字）', 'error');
return;
}
...
if (err) {
showMessage('セキュリティチャレンジを完了してください', 'error');
...
showMessage(result.error || 'メモの作成に失敗しました', 'error');
}
}
} catch (error) {
showMessage('メモ作成中にエラーが発生しました', 'error');
} finally {
submitButton.disabled = false;
submitButton.textContent = 'セキュアメモを作成';
...
showMessage('⚠️ 手動でコピーしてください (Ctrl+C / Cmd+C)', 'warning');
}
});
...
showMessage('✅ パスワードをクリップボードにコピーしました！', 'success');
