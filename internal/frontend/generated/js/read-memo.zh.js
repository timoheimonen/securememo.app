...
MISSING_MEMO_ID: '无效请求。请检查您的输入并重试。',
MISSING_PASSWORD: '请输入加密密码',
INVALID_MEMO_URL: '无效的备忘录 URL',
MISSING_SECURITY_CHALLENGE: '请完成安全验证',
MEMO_ALREADY_READ_DELETED: '此备忘录已被阅读并删除，或已过期。',
MEMO_EXPIRED_DELETED: '此备忘录已过期并被删除。',
INVALID_PASSWORD_CHECK: '密码无效。请检查您单独收到的密码。',
READ_MEMO_ERROR: '阅读备忘录时发生错误',
DECRYPTION_ERROR: '解密消息失败。密码无效或数据损坏。'
};
...
if (memoStatus) {
memoStatus.textContent = '备忘录已解密。正在删除中...请稍候。';
}
if (deletionSpinner) {
...
showMessage('请求过多。请稍等片刻后重试。', 'error');
const deletionSpinner = document.getElementById('deletionSpinner');
if (deletionSpinner) {
...
const deletionSpinner = document.getElementById('deletionSpinner');
if (memoStatus) {
memoStatus.textContent = '备忘录已确认阅读并永久删除。';
}
if (deletionSpinner) {
deletionSpinner.style.display = 'none';
}
} else {
showMessage('确认删除时出错。备忘录将自动清理。', 'warning');
const deletionSpinner = document.getElementById('deletionSpinner');
if (deletionSpinner) {
deletionSpinner.style.display = 'none';
}
...
decryptButton.textContent = '解密备忘录';
}
if (decryptLoadingIndicator) {
