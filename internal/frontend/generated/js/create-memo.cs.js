...
if (!message) {
showMessage('Zadejte prosím zprávu', 'error');
return;
}
if (message.length > 10000) {
showMessage('Zpráva je příliš dlouhá (max. 10 000 znaků)', 'error');
return;
}
...
if (err) {
showMessage('Dokončete prosím bezpečnostní ověření', 'error');
...
submitButton.textContent = 'Šifrování...';
...
showMessage(result.error || 'Nepodařilo se vytvořit zprávu', 'error');
}
}
} catch (error) {
showMessage('Při vytváření zprávy došlo k chybě', 'error');
} finally {
submitButton.disabled = false;
submitButton.textContent = 'Vytvořit zabezpečenou zprávu';
...
showMessage('✅ URL zkopírována do schránky!', 'success');
...
showMessage('⚠️ Zkopírujte prosím ručně (Ctrl+C / Cmd+C)', 'warning');
}
});
...
showMessage('✅ Heslo zkopírováno do schránky!', 'success');
