...
MISSING_MEMO_ID: '잘못된 요청입니다. 입력을 확인하고 다시 시도하세요.',
MISSING_PASSWORD: '암호화 비밀번호를 입력해 주세요',
INVALID_MEMO_URL: '잘못된 메모 URL입니다',
MISSING_SECURITY_CHALLENGE: '보안 인증을 완료해 주세요',
MEMO_ALREADY_READ_DELETED: '이 메모는 이미 읽히고 삭제되었거나 만료되었습니다.',
MEMO_EXPIRED_DELETED: '이 메모는 만료되어 삭제되었습니다.',
INVALID_PASSWORD_CHECK: '잘못된 비밀번호입니다. 별도로 받은 비밀번호를 확인해 주세요.',
READ_MEMO_ERROR: '메모를 읽는 동안 오류가 발생했습니다',
DECRYPTION_ERROR: '메시지 복호화에 실패했습니다. 잘못된 비밀번호이거나 데이터가 손상되었습니다.'
};
...
decryptButton.textContent = '복호화 중...';
}
...
showError('요청이 너무 많습니다. 잠시 기다린 후 다시 시도하세요.');
return;
}
...
if (memoStatus) {
memoStatus.textContent = '메모가 복호화되었습니다. 삭제 진행 중... 잠시 기다려 주세요.';
}
if (deletionSpinner) {
...
showMessage('요청이 너무 많습니다. 잠시 기다린 후 다시 시도하세요.', 'error');
const deletionSpinner = document.getElementById('deletionSpinner');
if (deletionSpinner) {
...
const deletionSpinner = document.getElementById('deletionSpinner');
if (memoStatus) {
memoStatus.textContent = '메모가 읽혔음을 확인하고 영구적으로 삭제되었습니다.';
}
if (deletionSpinner) {
deletionSpinner.style.display = 'none';
}
} else {
showMessage('삭제 확인 중 오류가 발생했습니다. 메모는 자동으로 정리됩니다.', 'warning');
const deletionSpinner = document.getElementById('deletionSpinner');
if (deletionSpinner) {
deletionSpinner.style.display = 'none';
}
...
decryptButton.textContent = '메모 복호화';
}
if (decryptLoadingIndicator) {
