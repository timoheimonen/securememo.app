...
if (!message) {
showMessage('Пожалуйста, введите заметку', 'error');
return;
}
if (message.length > 10000) {
showMessage('Заметка слишком длинная (максимум 10 000 символов)', 'error');
return;
}
...
if (err) {
showMessage('Пожалуйста, выполните проверку безопасности', 'error');
...
submitButton.textContent = 'Шифрование...';
...
showMessage(result.error || 'Не удалось создать заметку', 'error');
}
}
} catch (error) {
showMessage('Произошла ошибка при создании заметки', 'error');
} finally {
submitButton.disabled = false;
submitButton.textContent = 'Создать защищённую заметку';
...
showMessage('⚠️ Пожалуйста, скопируйте вручную (Ctrl+C / Cmd+C)', 'warning');
}
});
