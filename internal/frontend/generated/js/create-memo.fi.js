...
if (!message) {
showMessage('Syötä muistio', 'error');
return;
}
if (message.length > 10000) {
showMessage('Muistio on liian pitkä (max 10 000 merkkiä)', 'error');
return;
}
...
if (err) {
showMessage('Täytä turvallisuushaaste', 'error');
...
submitButton.textContent = 'Salaamassa...';
...
showMessage(result.error || 'Muistion luonti epäonnistui', 'error');
}
}
} catch (error) {
showMessage('Virhe muistion luonnissa', 'error');
} finally {
submitButton.disabled = false;
submitButton.textContent = 'Luo Suojattu Muistio';
...
showMessage('✅ URL kopioitu leikepöydälle!', 'success');
...
showMessage('⚠️ Kopioi manuaalisesti (Ctrl+C / Cmd+C)', 'warning');
}
});
...
showMessage('✅ Salasana kopioitu leikepöydälle!', 'success');
