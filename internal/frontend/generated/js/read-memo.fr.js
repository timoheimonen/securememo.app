...
MISSING_MEMO_ID: 'Requête invalide. Veuillez vérifier votre saisie et réessayer.',
MISSING_PASSWORD: 'Veuillez entrer le mot de passe de chiffrement',
INVALID_MEMO_URL: 'URL de mémo invalide',
MISSING_SECURITY_CHALLENGE: 'Veuillez compléter le défi de sécurité',
MEMO_ALREADY_READ_DELETED: 'Ce mémo a déjà été lu et supprimé, ou il a expiré.',
MEMO_EXPIRED_DELETED: 'Ce mémo a expiré et a été supprimé.',
INVALID_PASSWORD_CHECK: 'Mot de passe invalide. Veuillez vérifier le mot de passe que vous avez reçu séparément.',
READ_MEMO_ERROR: 'Une erreur s\'est produite lors de la lecture du mémo',
DECRYPTION_ERROR: 'Échec du déchiffrement du message. Mot de passe invalide ou données corrompues.'
};
...
decryptButton.textContent = 'Déchiffrement...';
}
...
showError('Trop de demandes. Veuillez attendre un moment et réessayer.');
return;
}
...
if (memoStatus) {
memoStatus.textContent = 'Mémo déchiffré. Suppression en cours... Veuillez patienter.';
}
if (deletionSpinner) {
...
showMessage('Trop de demandes. Veuillez attendre un moment et réessayer.', 'error');
const deletionSpinner = document.getElementById('deletionSpinner');
if (deletionSpinner) {
...
const deletionSpinner = document.getElementById('deletionSpinner');
if (memoStatus) {
memoStatus.textContent = 'Mémo confirmé comme lu et supprimé définitivement.';
}
if (deletionSpinner) {
deletionSpinner.style.display = 'none';
}
} else {
showMessage('Erreur lors de la confirmation de suppression. Le mémo sera nettoyé automatiquement.', 'warning');
const deletionSpinner = document.getElementById('deletionSpinner');
if (deletionSpinner) {
deletionSpinner.style.display = 'none';
}
...
decryptButton.textContent = 'Déchiffrer le Mémo';
}
if (decryptLoadingIndicator) {
