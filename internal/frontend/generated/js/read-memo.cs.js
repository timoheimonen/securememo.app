...
MISSING_MEMO_ID: 'Neplatný požadavek. Zkontrolujte prosím vstup a zkuste to znovu.',
MISSING_PASSWORD: 'Zadejte prosím šifrovací heslo',
INVALID_MEMO_URL: 'Neplatná URL zprávy',
MISSING_SECURITY_CHALLENGE: 'Dokončete prosím bezpečnostní ověření',
MEMO_ALREADY_READ_DELETED: 'Tato zpráva již byla přečtena a smazána, nebo jí vypršela platnost.',
MEMO_EXPIRED_DELETED: 'Této zprávě vypršela platnost a byla smazána.',
INVALID_PASSWORD_CHECK: 'Neplatné heslo. Zkontrolujte heslo, které jste obdrželi zvlášť.',
READ_MEMO_ERROR: 'Při čtení zprávy došlo k chybě',
DECRYPTION_ERROR: 'Nepodařilo se dešifrovat zprávu. Neplatné heslo nebo poškozená data.'
};
...
decryptButton.textContent = 'Dešifrování...';
}
...
showError('Příliš mnoho požadavků. Počkejte prosím a zkuste to znovu.');
return;
}
...
if (memoStatus) {
memoStatus.textContent = 'Zpráva dešifrována. Probíhá mazání... Počkejte prosím.';
}
if (deletionSpinner) {
...
showMessage('Příliš mnoho požadavků. Počkejte prosím a zkuste to znovu.', 'error');
const deletionSpinner = document.getElementById('deletionSpinner');
if (deletionSpinner) {
...
const deletionSpinner = document.getElementById('deletionSpinner');
if (memoStatus) {
memoStatus.textContent = 'Zpráva potvrzena jako přečtená a trvale smazána.';
}
if (deletionSpinner) {
deletionSpinner.style.display = 'none';
}
} else {
showMessage('Chyba při potvrzení smazání. Zpráva bude automaticky odstraněna.', 'warning');
const deletionSpinner = document.getElementById('deletionSpinner');
if (deletionSpinner) {
deletionSpinner.style.display = 'none';
}
...
decryptButton.textContent = 'Dešifrovat zprávu';
}
if (decryptLoadingIndicator) {
...
toggleBtn.textContent = 'Zobrazit';
toggleBtn.style.backgroundColor = '#007bff';
}
});
}
