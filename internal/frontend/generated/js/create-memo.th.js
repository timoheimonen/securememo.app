...
if (!message) {
showMessage('โปรดกรอกบันทึก', 'error');
return;
}
if (message.length > 10000) {
showMessage('บันทึกยาวเกินไป (สูงสุด 10,000 อักขระ)', 'error');
return;
}
...
if (err) {
showMessage('โปรดทำแบบทดสอบความปลอดภัยให้เสร็จ', 'error');
...
submitButton.textContent = 'กำลังเข้ารหัส...';
...
showMessage(result.error || 'สร้างบันทึกไม่สำเร็จ', 'error');
}
}
} catch (error) {
showMessage('เกิดข้อผิดพลาดระหว่างการสร้างบันทึก', 'error');
} finally {
submitButton.disabled = false;
submitButton.textContent = 'สร้างบันทึกลับที่ปลอดภัย';
...
showMessage('✅ คัดลอก URL ไปยังคลิปบอร์ดแล้ว!', 'success');
...
showMessage('⚠️ โปรดคัดลอกด้วยตนเอง (Ctrl+C / Cmd+C)', 'warning');
}
});
...
showMessage('✅ คัดลอกรหัสผ่านไปยังคลิปบอร์ดแล้ว!', 'success');
