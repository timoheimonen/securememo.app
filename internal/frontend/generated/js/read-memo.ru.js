...
MISSING_MEMO_ID: 'Некорректный запрос. Пожалуйста, проверьте введённые данные и попробуйте снова.',
MISSING_PASSWORD: 'Пожалуйста, введите пароль для шифрования',
INVALID_MEMO_URL: 'Некорректная ссылка на заметку',
MISSING_SECURITY_CHALLENGE: 'Пожалуйста, выполните проверку безопасности',
MEMO_ALREADY_READ_DELETED: 'Эта заметка уже была прочитана и удалена или истек срок её хранения.',
MEMO_EXPIRED_DELETED: 'Срок хранения заметки истёк, она была удалена.',
INVALID_PASSWORD_CHECK: 'Неверный пароль. Пожалуйста, проверьте пароль, который вы получили отдельно.',
READ_MEMO_ERROR: 'Произошла ошибка при чтении заметки',
DECRYPTION_ERROR: 'Не удалось расшифровать сообщение. Неверный пароль или повреждённые данные.'
};
...
decryptButton.textContent = 'Расшифровка...';
}
...
showError('Слишком много запросов. Пожалуйста, подождите и попробуйте снова.');
return;
}
...
if (memoStatus) {
memoStatus.textContent = 'Заметка расшифрована. Удаление... Пожалуйста, подождите.';
}
if (deletionSpinner) {
...
showMessage('Слишком много запросов. Пожалуйста, подождите и попробуйте снова.', 'error');
const deletionSpinner = document.getElementById('deletionSpinner');
if (deletionSpinner) {
...
const deletionSpinner = document.getElementById('deletionSpinner');
if (memoStatus) {
memoStatus.textContent = 'Заметка подтверждена как прочитанная и удалена навсегда.';
}
if (deletionSpinner) {
deletionSpinner.style.display = 'none';
}
} else {
showMessage('Ошибка подтверждения удаления. Заметка будет удалена автоматически.', 'warning');
const deletionSpinner = document.getElementById('deletionSpinner');
if (deletionSpinner) {
deletionSpinner.style.display = 'none';
}
...
decryptButton.textContent = 'Расшифровать заметку';
}
if (decryptLoadingIndicator) {
