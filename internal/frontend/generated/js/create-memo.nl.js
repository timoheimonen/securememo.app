...
if (!message) {
showMessage('Voer een memo in', 'error');
return;
}
if (message.length > 10000) {
showMessage('Memo is te lang (maximaal 10.000 tekens)', 'error');
return;
}
...
if (err) {
showMessage('Voltooi de beveiligingscontrole', 'error');
...
submitButton.textContent = 'Bezig met versleutelen...';
...
showMessage(result.error || 'Memo maken mislukt', 'error');
}
}
} catch (error) {
showMessage('Er is een fout opgetreden bij het maken van de memo', 'error');
} finally {
submitButton.disabled = false;
submitButton.textContent = 'Veilig Memo maken';
...
showMessage('✅ URL gekopieerd naar klembord!', 'success');
...
showMessage('⚠️ Kopieer handmatig (Ctrl+C / Cmd+C)', 'warning');
}
});
...
showMessage('✅ Wachtwoord gekopieerd naar klembord!', 'success');
