...
if (!message) {
showMessage('Vänligen ange en memo', 'error');
return;
}
if (message.length > 10000) {
showMessage('Memo är för lång (max 10 000 tecken)', 'error');
return;
}
...
if (err) {
showMessage('Vänligen slutför säkerhetsutmaningen', 'error');
...
submitButton.textContent = 'Krypterar...';
...
showMessage(result.error || 'Det gick inte att skapa memo', 'error');
}
}
} catch (error) {
showMessage('Ett fel uppstod vid skapandet av memo', 'error');
} finally {
submitButton.disabled = false;
submitButton.textContent = 'Skapa säker memo';
...
showMessage('✅ URL kopierad till urklipp!', 'success');
...
showMessage('⚠️ Kopiera manuellt (Ctrl+C / Cmd+C)', 'warning');
}
});
...
showMessage('✅ Lösenord kopierat till urklipp!', 'success');
