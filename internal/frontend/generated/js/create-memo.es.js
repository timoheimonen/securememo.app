...
if (!message) {
showMessage('Por favor ingresa un memo', 'error');
return;
}
if (message.length > 10000) {
showMessage('El memo es demasiado largo (máx 10,000 caracteres)', 'error');
return;
}
...
if (err) {
showMessage('Por favor completa el desafío de seguridad', 'error');
...
submitButton.textContent = 'Cifrando...';
...
showMessage(result.error || 'Falló al crear memo', 'error');
}
}
} catch (error) {
showMessage('Ocurrió un error al crear el memo', 'error');
} finally {
submitButton.disabled = false;
submitButton.textContent = 'Crear Memo Seguro';
...
showMessage('⚠️ Por favor, copia manualmente (Ctrl+C / Cmd+C)', 'warning');
}
});
