...
if (!message) {
showMessage('Indtast venligst et notat', 'error');
return;
}
if (message.length > 10000) {
showMessage('Notatet er for langt (maks. 10.000 tegn)', 'error');
return;
}
...
if (err) {
showMessage('Udfyld venligst sikkerhedsudfordringen', 'error');
...
submitButton.textContent = 'Krypterer...';
...
showMessage(result.error || 'Kunne ikke oprette notat', 'error');
}
}
} catch (error) {
showMessage('Der opstod en fejl under oprettelse af notatet', 'error');
} finally {
submitButton.disabled = false;
submitButton.textContent = 'Opret Sikkert Notat';
...
showMessage('✅ URL kopieret til udklipsholder!', 'success');
...
showMessage('⚠️ Kopiér venligst manuelt (Ctrl+C / Cmd+C)', 'warning');
}
});
...
showMessage('✅ Adgangskode kopieret til udklipsholder!', 'success');
