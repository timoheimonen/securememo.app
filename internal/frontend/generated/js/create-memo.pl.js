...
if (!message) {
showMessage('Proszę wprowadzić notatkę', 'error');
return;
}
if (message.length > 10000) {
showMessage('Notatka jest za długa (maks. 10 000 znaków)', 'error');
return;
}
...
if (err) {
showMessage('Proszę ukończyć wyzwanie bezpieczeństwa', 'error');
...
submitButton.textContent = 'Szyfrowanie...';
...
showMessage(result.error || 'Nie udało się utworzyć notatki', 'error');
}
}
} catch (error) {
showMessage('Wystąpił błąd podczas tworzenia notatki', 'error');
} finally {
submitButton.disabled = false;
submitButton.textContent = 'Utwórz bezpieczną notatkę';
...
showMessage('✅ URL skopiowano do schowka!', 'success');
...
showMessage('⚠️ Proszę skopiować ręcznie (Ctrl+C / Cmd+C)', 'warning');
}
});
...
showMessage('✅ Hasło skopiowano do schowka!', 'success');
