...
if (!message) {
showMessage('Por favor, digite um memo', 'error');
return;
}
if (message.length > 10000) {
showMessage('Memo muito longo (máx 10.000 caracteres)', 'error');
return;
}
...
if (err) {
showMessage('Por favor, complete o desafio de segurança', 'error');
...
showMessage(result.error || 'Falha ao criar memo', 'error');
}
}
} catch (error) {
showMessage('Ocorreu um erro ao criar o memo', 'error');
} finally {
submitButton.disabled = false;
submitButton.textContent = 'Criar Memo Seguro';
...
showMessage('⚠️ Por favor, copie manualmente (Ctrl+C / Cmd+C)', 'warning');
}
});
...
showMessage('✅ Palavra-passe copiada para a área de transferência!', 'success');
