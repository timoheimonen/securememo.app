...
if (!message) {
showMessage('Vennligst skriv inn et notat', 'error');
return;
}
...
if (err) {
showMessage('Vennligst fullfør sikkerhetsutfordringen', 'error');
...
showMessage(result.error || 'Kunne ikke opprette notat', 'error');
}
}
} catch (error) {
showMessage('En feil oppstod under oppretting av notatet', 'error');
} finally {
submitButton.disabled = false;
submitButton.textContent = 'Opprett sikkert notat';
...
showMessage('✅ URL kopiert til utklippstavlen!', 'success');
...
showMessage('⚠️ Vennligst kopier manuelt (Ctrl+C / Cmd+C)', 'warning');
}
});
...
showMessage('✅ Passord kopiert til utklippstavlen!', 'success');
