...
if (!message) {
showMessage('Введіть записку', 'error');
return;
}
if (message.length > 10000) {
showMessage('Записка занадто довга (максимум 10 000 символів)', 'error');
return;
}
...
if (err) {
showMessage('Будь ласка, виконайте перевірку безпеки', 'error');
...
submitButton.textContent = 'Шифрування...';
...
showMessage(result.error || 'Не вдалося створити записку', 'error');
}
}
} catch (error) {
showMessage('Виникла помилка під час створення записки', 'error');
} finally {
submitButton.disabled = false;
submitButton.textContent = 'Створити захищену записку';
...
showMessage('✅ URL скопійовано в буфер обміну!', 'success');
...
showMessage('⚠️ Будь ласка, скопіюйте вручну (Ctrl+C / Cmd+C)', 'warning');
}
});
...
showMessage('✅ Пароль скопійовано в буфер обміну!', 'success');
