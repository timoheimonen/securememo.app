...
if (!message) {
showMessage('Kérjük, adja meg a jegyzetet', 'error');
return;
}
if (message.length > 10000) {
showMessage('A jegyzet túl hosszú (max. 10 000 karakter)', 'error');
return;
}
...
if (err) {
showMessage('Kérjük, teljesítse a biztonsági ellenőrzést', 'error');
...
submitButton.textContent = 'Titkosítás folyamatban...';
...
showMessage(result.error || 'Nem sikerült létrehozni a jegyzetet', 'error');
}
}
} catch (error) {
showMessage('Hiba történt a jegyzet létrehozásakor', 'error');
} finally {
submitButton.disabled = false;
submitButton.textContent = 'Biztonságos jegyzet létrehozása';
...
showMessage('✅ Az URL vágólapra másolva!', 'success');
...
showMessage('⚠️ Kérjük, másold manuálisan (Ctrl+C / Cmd+C)', 'warning');
}
});
...
showMessage('✅ A jelszó vágólapra másolva!', 'success');
