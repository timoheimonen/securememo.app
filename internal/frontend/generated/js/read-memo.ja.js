...
MISSING_MEMO_ID: '無効なリクエストです。入力内容を確認して再試行してください。',
MISSING_PASSWORD: '暗号化パスワードを入力してください',
INVALID_MEMO_URL: '無効なメモURL',
MISSING_SECURITY_CHALLENGE: 'セキュリティチャレンジを完了してください',
MEMO_ALREADY_READ_DELETED: 'このメモはすでに読み取り済みで削除されたか、期限切れです。',
MEMO_EXPIRED_DELETED: 'このメモは期限切れで削除されました。',
INVALID_PASSWORD_CHECK: '無効なパスワードです。別途受け取ったパスワードを確認してください。',
READ_MEMO_ERROR: 'メモ読み取り中にエラーが発生しました',
DECRYPTION_ERROR: 'メッセージの復号化に失敗しました。パスワードが無効かデータが破損しています。'
};
...
decryptButton.textContent = '復号化中...';
}
...
showError('リクエストが多すぎます。少しお待ちいただいてから再度お試しください。');
return;
}
...
if (memoStatus) {
memoStatus.textContent = 'メモが復号化されました。削除中です...お待ちください。';
}
if (deletionSpinner) {
...
showMessage('リクエストが多すぎます。少しお待ちいただいてから再度お試しください。', 'error');
const deletionSpinner = document.getElementById('deletionSpinner');
if (deletionSpinner) {
...
const deletionSpinner = document.getElementById('deletionSpinner');
if (memoStatus) {
memoStatus.textContent = 'メモが読まれたことが確認され、完全に削除されました。';
}
if (deletionSpinner) {
deletionSpinner.style.display = 'none';
}
} else {
showMessage('削除の確認中にエラーが発生しました。メモは自動的にクリーンアップされます。', 'warning');
const deletionSpinner = document.getElementById('deletionSpinner');
if (deletionSpinner) {
deletionSpinner.style.display = 'none';
}
...
decryptButton.textContent = 'メモを復号化';
}
if (decryptLoadingIndicator) {
