...
READ_MEMO_ERROR: 'A apărut o eroare la citirea memoului',
DECRYPTION_ERROR: 'Decriptarea mesajului a eșuat. Parolă invalidă sau date corupte.'
};
...
decryptButton.textContent = 'Se decriptează...';
}
...
showError('Prea multe cereri. Așteaptă puțin și încearcă din nou.');
return;
}
...
if (memoStatus) {
memoStatus.textContent = 'Memo decriptat. Ștergere în curs... Te rugăm să aștepți.';
}
if (deletionSpinner) {
...
const deletionSpinner = document.getElementById('deletionSpinner');
if (memoStatus) {
memoStatus.textContent = 'Memoul a fost confirmat ca citit și șters definitiv.';
}
if (deletionSpinner) {
deletionSpinner.style.display = 'none';
}
} else {
showMessage('Eroare la confirmarea ștergerii. Memoul va fi curățat automat.', 'warning');
const deletionSpinner = document.getElementById('deletionSpinner');
if (deletionSpinner) {
deletionSpinner.style.display = 'none';
}
...
decryptButton.textContent = 'Decriptează memo-ul';
}
if (decryptLoadingIndicator) {
