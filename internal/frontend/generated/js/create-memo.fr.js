...
if (!message) {
showMessage('Veuillez entrer un mémo', 'error');
return;
}
if (message.length > 10000) {
showMessage('Le mémo est trop long (max 10 000 caractères)', 'error');
return;
}
...
if (err) {
showMessage('Veuillez compléter le défi de sécurité', 'error');
...
submitButton.textContent = 'Chiffrement...';
...
showMessage(result.error || 'Échec de création du mémo', 'error');
}
}
} catch (error) {
showMessage('Une erreur s\'est produite lors de la création du mémo', 'error');
} finally {
submitButton.disabled = false;
submitButton.textContent = 'Créer un Mémo Sécurisé';
...
showMessage('⚠️ Veuillez copier manuellement (Ctrl+C / Cmd+C)', 'warning');
}
});
...
showMessage('✅ Mot de passe copié dans le presse-papiers !', 'success');
