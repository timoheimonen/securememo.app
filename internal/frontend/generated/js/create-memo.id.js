...
if (!message) {
showMessage('Silakan masukkan memo', 'error');
return;
}
if (message.length > 10000) {
showMessage('Memo terlalu panjang (maks 10.000 karakter)', 'error');
return;
}
...
if (err) {
showMessage('Silakan selesaikan tantangan keamanan', 'error');
...
submitButton.textContent = 'Mengenkripsi...';
...
showMessage(result.error || 'Gagal membuat memo', 'error');
}
}
} catch (error) {
showMessage('Terjadi kesalahan saat membuat memo', 'error');
} finally {
submitButton.disabled = false;
submitButton.textContent = 'Buat Memo Aman';
...
showMessage('✅ URL berhasil disalin ke clipboard!', 'success');
...
showMessage('⚠️ Silakan salin secara manual (Ctrl+C / Cmd+C)', 'warning');
}
});
...
showMessage('✅ Kata sandi berhasil disalin ke clipboard!', 'success');
