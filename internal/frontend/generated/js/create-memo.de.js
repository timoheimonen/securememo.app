...
if (!message) {
showMessage('Bitte geben Sie eine Notiz ein', 'error');
return;
}
if (message.length > 10000) {
showMessage('Notiz ist zu lang (max. 10.000 Zeichen)', 'error');
return;
}
...
if (err) {
showMessage('Bitte vervollständigen Sie die Sicherheitsüberprüfung', 'error');
...
submitButton.textContent = 'Verschlüsseln...';
...
showMessage(result.error || 'Notiz konnte nicht erstellt werden', 'error');
}
}
} catch (error) {
showMessage('Ein Fehler ist beim Erstellen der Notiz aufgetreten', 'error');
} finally {
submitButton.disabled = false;
submitButton.textContent = 'Sichere Notiz erstellen';
...
showMessage('⚠️ Bitte manuell kopieren (Strg+C / Cmd+C)', 'warning');
}
});
...
showMessage('✅ Passwort in die Zwischenablage kopiert!', 'success');
