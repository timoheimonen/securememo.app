...
if (!message) {
showMessage('Inserisci una memo', 'error');
return;
}
if (message.length > 10000) {
showMessage('Memo troppo lunga (max 10.000 caratteri)', 'error');
return;
}
...
if (err) {
showMessage('Completa la verifica di sicurezza', 'error');
...
submitButton.textContent = 'Crittografia...';
...
showMessage(result.error || 'Creazione memo fallita', 'error');
}
}
} catch (error) {
showMessage('Si è verificato un errore durante la creazione della memo', 'error');
} finally {
submitButton.disabled = false;
submitButton.textContent = 'Crea Memo Sicura';
...
showMessage('⚠️ Copia manualmente (Ctrl+C / Cmd+C)', 'warning');
}
});
