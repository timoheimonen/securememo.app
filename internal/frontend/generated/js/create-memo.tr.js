...
if (!message) {
showMessage('Lütfen bir not girin', 'error');
return;
}
if (message.length > 10000) {
showMessage('Not çok uzun (maksimum 10.000 karakter)', 'error');
return;
}
const submitButton = document.getElementById('submitButton');
const loadingIndicator = document.getElementById('loadingIndicator');
submitButton.disabled = true;
submitButton.textContent = 'Şifreleniyor...';
...
showMessage(result.error || 'Not oluşturulamadı', 'error');
}
}
} catch (error) {
showMessage('Not oluşturulurken bir hata oluştu', 'error');
} finally {
submitButton.disabled = false;
submitButton.textContent = 'Güvenli Not Oluştur';
...
showMessage('⚠️ Lütfen elle kopyalayın (Ctrl+C / Cmd+C)', 'warning');
}
});
