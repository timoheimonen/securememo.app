...
if (!message) {
showMessage('请输入备忘录', 'error');
return;
}
if (message.length > 10000) {
showMessage('备忘录过长（最多10,000个字符）', 'error');
return;
}
...
showMessage(result.error || '创建备忘录失败', 'error');
}
}
} catch (error) {
showMessage('创建备忘录时发生错误', 'error');
} finally {
submitButton.disabled = false;
submitButton.textContent = '创建安全备忘录';
...
showMessage('⚠️ 请手动复制 (Ctrl+C / Cmd+C)', 'warning');
}
});
